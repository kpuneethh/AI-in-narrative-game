...
def story_path(story_options, user_input):
    """returns the chosen story path"""
    for key, option in story_options.items():
...
def create_prompt(last_input, options_dictionary, debug: bool = DEFAULT_DEBUG_VALUE):
    ...
    options = list(story_points_dictionary(options_dictionary).items())[1:]
...
def is_valid_response(ai_response: str, options_dictionary , num_of_valid_responses: int = None, debug: bool = DEFAULT_DEBUG_VALUE) -> bool:
    ...
    if debug:
        if ai_response not in story_points_dictionary(options_dictionary):
            logging.warning(f"the ai response ({ai_response}) was not valid")
    return ai_response in story_points_dictionary(options_dictionary)
...
def ai_story_generator(user_response : str, ai_prompt : str, dictionary_question : str, item_to_use : str, debug: bool = DEFAULT_DEBUG_VALUE):
    ...
    content = ''
    #content += 'The previous story included'
    content += f'The previous part of the story: {ai_prompt}\n'
    content += f'The current user response that you must transition to the story: {user_response}\n'
    content += f'The simplified story that this leads to: {dictionary_question}\n'
    content += f'Please expand the story taking into account the user input'
    if debug:
    ...
    if item_to_use:
        messages.append({"role": "user", "content": f"This item helped the user get through this situation, make sure to weave it with the story: {item_to_use}"})
    response = openai.ChatCompletion.create(
        model="gpt-4o-mini-2024-07-18",
        temperature = 1.0,
        max_tokens=300,
...
def generate_hint(user_response: str, story_dictionary: dict, item_to_use : str, debug: bool = DEFAULT_DEBUG_VALUE):
    ...
    response = openai.ChatCompletion.create(
        ...
        "content": (
            ...
            "Respond like the user is really in the situation, and dont call it a story"
            "Use 2nd person POV only. "
            "Dont call it a 'response'"
        )
        },
        {"role": "assistant", "content" : story_dictionary},
        {"role": "user", "content": user_response},
        {"role": "user", "content": f"The user needs this item to get through this situation, let them know that they need it (ignore if blank): {item_to_use}"}
        ]
    )
    return response.choices[0].message['content']